"""
Dungeon Guardian - An Intelligent Autonomous NPC Agent
Combines GOAP planning with LLM-style reasoning for dungeon protection
"""
...
class WorldState:
    ...
    stamina: int = 20
    potion_count: int = 0
    treasure_threat_level: str = "low"  # low, medium, high
...
class GOAPPlanner:
    """GOAP-based planner using A* algorithm"""
...
if __name__ == "__main__":
    ...
    custom_state = WorldState(
        health=45,           # Set custom health
        stamina=12,          # Set custom stamina
        potion_count=1,      # Number of potions
        has_potion=True,     # Whether agent has a potion ready
        treasure_threat_level="high",  # low/medium/high
        enemy_nearby=True,   # True/False
        is_in_safe_zone=False,  # True/False
        backup_available=True   # True/False
    )
    
    print("\n🔧 RUNNING CUSTOM SCENARIO")
    guardian = DungeonGuardian()
    guardian.run_simulation(custom_state, max_steps=10)